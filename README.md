...
## REST API инструкции:
- **[POST]** /api/clients - Создание нового клиента. В теле запроса должны быть переданы все необходимые данные для создания нового клиента.
- **[PUT]** /api/clients/{uuid} - Полное обновление данных клиента по указанному uuid. Если клиент с данным uuid не существует, будет создан новый клиент с указанными данными.
- **[PATCH]** /api/clients/{uuid} - Частичное обновление данных клиента по указанному uuid. Если клиент с данным uuid не найден, сервер вернет ошибку с кодом 404. Обновляются только переданные данные.
- **[GET]** /api/clients/{uuid}/loan-eligibility - Проверка кредитоспособности клиента по его uuid. В ответе будет указано, может ли клиент получить кредит на основании предоставленных данных.
